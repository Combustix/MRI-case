...
Generate_Patients <- function() {
  ...
  return(patient_data_comb)
}

Scheduling <- function(patient_data_comb, time_slot1, time_slot2) {  
  #For the machines
  ...
  #Assigns patients to schedule with desired timeslots in schedule
    for (i in 1:nrow(patient_data_comb)) {
      ...
      a1 <- available_slots[1]
      a2 <- available_slots2[1]
      
      if(length(available_slots2) < length(available_slots)){
        if(patient_data_comb$Type[i] == 1) {
          schedule$Patient[a1] <- patient
          schedule <- schedule[-((a1+1):(a1+time_slot_duration_type1-1)),]
        }
        else {
          schedule$Patient[a1] <- patient
          schedule <- schedule[-((a1+1):(a1+time_slot_duration_type2-1)),]
        }
      }
      else{
        if(patient_data_comb$Type[i] == 1) {
          schedule2$Patient[a2] <- patient
          schedule2 <- schedule2[-((a2+1):(a2+time_slot_duration_type1-1)),]
        }
        else {
          schedule2$Patient[a2] <- patient
          schedule2 <- schedule2[-((a2+1):(a2+time_slot_duration_type2-1)),]
        }
      }
    }
...
stats <- data.frame(mean1_deviation = numeric(0), mean2_deviation = numeric(0), idle_time = numeric(0), waiting_time = numeric(0), overtime = numeric(0), patients = numeric(0))
for(k in 0:(n-1)) {
  set.seed(seed + k)
  patients <- Generate_Patients()
  for(i in sequence) {
    for(j in sequence) {
      output <- Scheduling(patients, mean_duration1 + i, mean_duration2 + j)
      output <- append(output, list(i, j), after=0)
      names(output) <- c("mean1_deviation", "mean2_deviation", "idle_time", "waiting_time", "overtime", "patients")
...
for(i in sequence) {
  for(j in sequence) {
    sum_list <- c(sum(stats[which(stats[,1] == i & stats[,2] == j), 3])/n, sum(stats[which(stats[,1] == i & stats[,2] == j), 4])/n, 
                  sum(stats[which(stats[,1] == i & stats[,2] == j), 5])/n, sum(stats[which(stats[,1] == i & stats[,2] == j), 6])/n)
