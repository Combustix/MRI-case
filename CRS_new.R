...
patient_data <- data.frame(Call_Time = numeric(0), duration = numeric(0))
current_time <- 0
...
while (current_time < work_hours) {
  ...
  if (current_time < work_hours) {
    ...
    patient_data <- rbind(patient_data, data.frame(Call_Time = formatted_time, Duration = duration))
  }
}
...
patient_data2 <- data.frame(Call_Time = numeric(0), Duration = numeric(0))
current_time2 <- 0
...
while (current_time2 < work_hours2) {
  ...
  if (current_time2 < work_hours2) {
    ...
    patient_data2 <- rbind(patient_data2, data.frame(Call_Time = formatted_time2, Duration = duration2))
  }
}
...
time_sequence2 <- seq(start_time, end_time, by = time_slot_duration * 60)
...
schedule2 <- data.frame(Time = format(time_sequence2, format = "%H:%M"), Patient = NA)
