...
mean_duration <- ceiling((gshape/grate) * 60)
...
Generate_Patients <- function() {
  ...
  while (current_time < work_hours) {
    ...
    if (current_time < work_hours) {
      ...
      patient_data<- rbind(patient_data, data.frame(Call_Time = formatted_time, Duration = duration, Type = 2))
    }
  }
...
for(k in 0:(n-1)) {
  ...
  for(i in sequence) {
    output <- Scheduling(patients, mean_duration + i)
...
# for(k in 0:n-1) {
#   set.seed(seed + k)
#   stats <- rbind(stats, DES(mean_duration + 10, mean_duration2 + 10))
# }
